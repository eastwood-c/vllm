...
class PPHandler:
    ...
    def broadcast(
        ...
        # Ordered AFTER the pad above: the pad is a main-stream op, so a sync
        # placed before it would not cover the copy the broadcast then reads.
        if current_platform.is_xpu():
            self.main_stream.synchronize()

        # Build the packed row (see __init__ for the layout) on the main stream.
        # The packed buffer is always full width, which is also what keeps the
        # NCCL element count matching the receiver on EVERY step: the sampler
        # emits width 1 on steps with no draft tokens (prefill, first decode)
        # and width (num_spec+1) only once rejection sampling has run, and an
        # unpadded width-1 send against a width-(num_spec+1) recv deadlocks the
        # receiver. The unused sampled columns stay -1, placeholders ignored by
        # post_update, which advances each request by its per-request
        # num_sampled valid count. Draft columns (spec decode) are filled by
        # broadcast_draft() after propose(); the single NCCL op is issued there
        # so there is exactly ONE broadcast per step.
        msl = self.max_sample_len
